...
get_md <- function(path_ecmd = NULL, path_rawdata = NULL, path_output = NULL, 
                   online, path_sa_file = NULL, path_pf_file = NULL, tlag_meth, 
  ...
  if (md["GA_PATH"] == "closed") {
    ...
    for (h in 1:nh) {
      if (h == 1) {
        var_ep[h] <- "u"
        instr_ep[h] <- md["SA_MODEL"]
        unit_ep[h] <- md["UVW_UNITS"]
      }
      if (h == 2) {
        var_ep[h] <- "v"
        instr_ep[h] <- md["SA_MODEL"]
      ...
      if (h == 4) {
        var_ep[h] <- "ts"
        instr_ep[h] <- md["SA_MODEL"]
        unit_ep[h] <- md["T_SONIC_UNITS"]
      }
      if (h == 5) {
        var_ep[h] <- "co2"
        instr_ep[h] <- md["GA_MODEL"]
        unit_ep[h] <- md["CO2_UNITS"]
        meas_type_ep[h] <- md["CO2_measure_type"]
      }
      if (h == 6) {
        var_ep[h] <- "h2o"
        instr_ep[h] <- md["GA_MODEL"]
        unit_ep[h] <- md["H2O_UNITS"]
        meas_type_ep[h] <- md["H2O_measure_type"]
      }
      if (h == 7) {
        var_ep[h] <- "not_numeric"
        instr_ep[h] <- md["SA_MODEL"]
        unit_ep[h] <- "dimensionless"
      }
      if (h == 8) {
        var_ep[h] <- "diag_72"
        instr_ep[h] <- md["GA_MODEL"]
