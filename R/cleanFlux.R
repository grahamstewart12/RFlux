...
clean_flux <- function(path_workset, path_ecmd, path_output = NULL,
  ...
  if (any(apply(matrix(NEE5, nrow = 48), MARGIN = 1, function(x) sum(is.na(x)) == (N / 48 - 3)))) {
    warning(call. = FALSE, "Too missing values for some half-hour in NEE flux variables! Outlier detection procedure described in Vitale et al (2019) is not performed")
  }
  if (any(apply(matrix(LE5, nrow = 48), MARGIN = 1, function(x) sum(is.na(x)) == (N / 48 - 3)))) {
    warning(call. = FALSE, "Too missing values for some half-hour in LE flux variables! Outlier detection procedure described in Vitale et al (2019) is not performed")
  }
  if (any(apply(matrix(H5, nrow = 48), MARGIN = 1, function(x) sum(is.na(x)) == (N / 48 - 3)))) {
    warning(call. = FALSE, "Too missing values for some half-hour in H flux variables! Outlier detection procedure described in Vitale et al (2019) is not performed")
  }
    ...
    for (flux in 1:3) {
      ...
      if (length(TS > 479) & all(apply(matrix(TS, nrow = 48), MARGIN = 1, function(x) length(which(is.na(x)))) < ceiling(N / 48))) {
        ...
        for (i in 1:(length(indq) - 1)) {
          ...
          if (length(na.omit(res[ind])) > 9) {
            asd <- aout.laplace(res[ind], c(median(res[ind], na.rm = TRUE), Qn(na.omit(res[ind]))), alpha)
            spike1 <- c(spike1, ind[which(asd$is.outlier)])
          }
          if (length(na.omit(res[ind])) < 10) {
            spike1 <- c(spike1, ind[which(!is.na(res[ind]))])
          } ## to prevent cases where missing values unable the application of aaut.laplace
        }
      }
      if (flux == 1) {
        spike1nee <- spike1
  ...
  set2exp <- data.frame(
    ...
    "SC" = replace(Sc, nee_ind_miss, NA),
    "NEE_UNCLEANED" = as.vector(replace(ec_data[, "CO2flux"] + Sc, nee_ind_miss, NA)),
  ...
  if (!is.null(path_output) & !is.null(file_name)) fwrite(set2exp, paste0(path_output, "/", file_name, ".csv"), sep = ",", quote = FALSE, col.names = TRUE, row.names = FALSE, na = "-9999")
  return(set2exp)
}
